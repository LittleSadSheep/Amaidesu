...
VLM 调用统一走 ``LLMManager.generate_vision(profile=VISION_PROFILE)``，key/model/重试/日志
走 model.toml 的 vision 档位与 ``[[llm_providers]]`` 池。
...
# 注：不依赖 aiohttp（VLM 调用统一走 LLMManager.generate_vision 封闭 vision 档位）。
# 图像以 bytes 形式传入 generate_vision，由客户端后端自动 data-URL 化。

# 本消费方绑定的 LLM 档位（代码显式声明，配置不承载绑定）。
VISION_PROFILE = "vision"

# VLM 提示词模板键（正文见包内 prompts/screen_vlm_prompt.md 与 screen_vlm_system.md，
# 由 src/**/prompts/ 约定目录内聚承载）。system message 与 user prompt 是 generate_vision
# 的两个独立参数，故拆为两个零变量模板。
...
class ScreenAnalysisResult:
    ...
    raw_response: dict[str, Any]  # VLM 原始响应（来自 payload.Response 字段）
...
class ScreenReader:
    ...
    - VLM 调用统一经由 :class:`LLMManager.generate_vision`，key/model/重试/日志由
      ``config/model.toml`` 的 vision 档位 + ``[[llm_providers]]`` 池统一管理
    """
    ...
    async def _call_vlm(self, image: Any, change_data: dict[str, Any]) -> Optional[ScreenAnalysisResult]:
        ...
        - 未注入 llm_manager → 返回带 "skipped" 标记的说明性 ScreenAnalysisResult
          （便于未配置 VLM 的场景仅做缓存去重）
        - llm_manager 注入但 generate_vision 失败 → 返回 None（异常分支由调用方按既有
          日志路径处理）
        """
        ...
        try:
            ...
            response = await self._llm_manager.generate_vision(
                self._get_vlm_prompt(),
                [image_bytes],
                profile=VISION_PROFILE,
                system=self._get_vlm_system_message(),
            )
        except Exception as e:
    ...
    def _image_to_bytes(image: Any) -> bytes:
        """PIL 图像 → PNG 字节流（供 LLMManager.generate_vision 作为 images 参数）。

        generate_vision 的客户端后端接受 ``bytes``，由
